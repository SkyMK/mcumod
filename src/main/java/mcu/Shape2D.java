...
public class Shape2D {
    ...
    public Shape3D extrude(float x, float y, float z, float rotX, float rotY, float rotZ, float depth, int u, int v,
                           float textureWidth, float textureHeight, int shapeTextureWidth, int shapeTextureHeight,
                           int sideTextureWidth, int sideTextureHeight, float[] faceLengths) {
        ...
        Vec3 extrudeVector = Vec3.createVectorHelper(0.0D, 0.0D, depth);
        ...
        for (int currentLengthPosition = 0; currentLengthPosition < this.coords.size(); ++currentLengthPosition) {
            ...
            verts[currentLengthPosition] = new PositionTransformVertex(x + (float) ratioLength.xCoord,
                    y + (float) ratioLength.yCoord, z + (float) ratioLength.zCoord, nextCoord, ratioPosition);
            verts[currentLengthPosition + this.coords.size()] = new PositionTransformVertex(
                    x + (float) ratioLength.xCoord - (float) extrudeVector.xCoord,
                    y + (float) ratioLength.yCoord - (float) extrudeVector.yCoord,
                    z + (float) ratioLength.zCoord - (float) extrudeVector.zCoord, currentLength, ratioPosition);
            vertsTop[currentLengthPosition] = new PositionTransformVertex(verts[currentLengthPosition]);
            vertsBottom[this.coords.size() - currentLengthPosition - 1] = new PositionTransformVertex(
                    verts[currentLengthPosition + this.coords.size()]);
            if (faceLengths != null) {
                totalLength += faceLengths[currentLengthPosition];
            } else {
                totalLength = (float) ((double) totalLength + Math.sqrt(
                        Math.pow(idx.xCoord - curCoord.xCoord, 2.0D) + Math.pow(idx.yCoord - curCoord.yCoord, 2.0D)));
            }
        }
        ...
        for (int var36 = 0; var36 < this.coords.size(); ++var36) {
            ...
            currentLength = (float) Math.sqrt(
                    Math.pow(curCoord.xCoord - var37.xCoord, 2.0D) + Math.pow(curCoord.yCoord - var37.yCoord, 2.0D));
            if (faceLengths != null) {
            ...
            PositionTransformVertex[] polySide = new PositionTransformVertex[]{
                    new PositionTransformVertex(verts[var36], texU2, texV1),
                    new PositionTransformVertex(verts[this.coords.size() + var36], texU2, texV2),
                    new PositionTransformVertex(verts[this.coords.size() + (var36 + 1) % this.coords.size()], texU1,
                            texV2),
                    new PositionTransformVertex(verts[(var36 + 1) % this.coords.size()], texU1, texV1)};
